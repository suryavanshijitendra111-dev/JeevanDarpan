...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    ROOTDIRSRAW=`find -L / -maxdepth 3 -type f -name gradlew -print0 2>/dev/null | tr '\0' '\n' | sed -n 1p`
    ROOTDIR=`dirname "$ROOTDIRSRAW"`
    ROOTDIR_UNIX=`expr "$ROOTDIR" : '\(.*\)' | sed 's|\\\\|/|g'`
    # Remove trailing slash
    if [ -z "${ROOTDIR_UNIX##*+(/|\\)}" ]; then
        ROOTDIR_UNIX=${ROOTDIR_UNIX%?(+(/|\\))}
    fi
    if [ -z "${CLASSPATH##*+(/|\\)}" ]; then
        CLASSPATH=${CLASSPATH%?(+(/|\\))}
    fi
